...
- **Keyboard shortcuts** (desktop):
  ...
  - `F` — toggle fullscreen
  - `T` — toggle theme
  - Shortcuts only fire while keyboard focus is inside the page's own document. Clicking an ad can move focus into that ad's iframe, after which keystrokes go there instead of to the player — indistinguishable from "the shortcuts stopped working." The page reclaims focus on load, whenever you interact with its own UI (excluding form fields/buttons/links/ads themselves), and shortly after an iframe takes it.
...
- **Four manual display ad units**: one at the **bottom** of the left sidebar (`#sidebar-ad-space`, desktop only, auto/responsive size, slot `2402890298` / "xjj-sidebar", with `margin-top: 24px` separating it from the Add Source button above — see [Resolved: ads not responding to clicks](#resolved-ads-not-responding-to-clicks)), one in a **dedicated right-side panel** (`#right-ad-panel`, desktop only, fixed 160×600, reusing the same slot `2402890298` — the same slot ID can serve different sizes/formats depending on each placement) mirroring the left sidebar's role as its own reserved column rather than an overlay, one inside the "take a break" modal (fixed 336×280, shown to every visitor, every 8 videos, on both desktop and mobile, slot `7463645282` / "XJJ - Break Modal", with `margin-bottom: 44px` separating it from the Stop/Continue buttons below), and a mobile-only banner (`#mobile-ad-banner`, fixed 320×50, fixed just below the top bar, slot `3800566595` / "XJJ - Mobile Banner") — previously mobile visitors only ever saw an ad in the break modal, since the sidebar (and its ads) are hidden entirely on mobile. The right panel ad previously lived at the bottom of the left sidebar; it moved to its own column specifically so it never overlaps the video, consistent with every other ad placement decision here.
- Three of the four ad units use a **fixed size** rather than `data-ad-format="auto"` + `data-full-width-responsive="true"`. Responsive slots let Google's ad server pick any format, including expandable/interactive rich-media creatives (e.g. an oversized skyscraper that pushed sidebar content off-screen, and "vignette"-style app-install/travel-tour ads that overlaid a large chunk of the video with a non-functional close button) — fixed IAB standard sizes constrain what can be served. The right-panel ad was briefly auto/responsive on the theory that being the last element in a scrollable sidebar made an oversized creative harmless — that turned out to be wrong: the expandable creative rendered as a viewport-relative overlay covering the video, ignoring where its container sits on the page entirely, so it was switched to fixed. The sidebar ad (`#sidebar-ad-space`) is currently auto/responsive again by request — if the oversized/expandable-overlay behavior reappears there, switching it back to a fixed size (e.g. `width:160px; height:300px`) is the known mitigation. If aggressive creatives still slip through regardless of size, use AdSense's **Ads → Content → Blocking controls** to block specific categories (e.g. "Dating") — that's account-level and outside what this codebase can control.
...
## Resolved: ads not responding to clicks

Ads rendered correctly but clicking them did nothing (no navigation, no popup, no popup-blocked indicator). Ruled out along the way: our own JS (no `preventDefault`/`stopPropagation`/`beforeunload`/`window.open` interference), an overlay of ours (DevTools confirmed hovering an ad selects the ad's own `<a>` tag), CSP/sandbox (none set), and the browser/ad blocker (ads on other sites clicked through fine in the same browser). A global CSS reset scoping fix was tried first (excluding `ins.adsbygoogle` from `*, *::before, *::after` — kept, since it's harmless and correct regardless) based on a since-superseded theory about a DOM-injected ad format.

**Actual root cause, found via DevTools:** the ad creative's own markup included `data-ifc-map="[[\"confirmedClickAdUnitProtection\", ...], [\"defaultAdUnitProtection\", [[[\"10,10,10,10\", ...]]]], ...]"` — this is **Google's own accidental-click protection**: a ~10px dead zone around the ad, plus a "confirmed click" mode where a click too close to *other interactive controls* only arms the ad rather than navigating on the first click. Every ad placement here originally sat right next to buttons/toggles (the sidebar ad above/below other controls, the break-modal ad directly above the Stop/Continue buttons) — exactly the geometry that triggers this. The fix was giving every ad slot genuine spacing from adjacent interactive elements (`margin-top: 24px` before the sidebar ad, `margin-bottom: 44px` on `.gate-ad-slot` before the break modal's buttons), not a JS/CSS workaround — there's no legitimate way to disable Google's own anti-fraud click protection, and trying to would risk the AdSense account (same reasoning as declining to patch around `#click-protector` earlier).
